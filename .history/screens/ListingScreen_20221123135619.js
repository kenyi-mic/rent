...
const ListingScreen = () => {
  ...
  const [postSuccess, setPostSucess] = useState("");
  ...
  const storeImageData = async () => {
    imageData.map(async (component, index) => {
      ...
      const extension = urlPath[urlPath.length - 1];
      const key = `${uuidv4()}.${extension}`;
      imageAllUri.push({ imageUri: key });
      await Storage.put(key, blob);
      setPostProcessing(true);
      setPostSucess("You have successfully published your Advertisement");
      if (imageData.length === index + 1) {
        const postData = {
          ...
          images: JSON.stringify(imageAllUri),
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <Pressable
          onPress={() => storeImageData()}
          android_ripple={{ color: "lightblue" }}
          className="m-8 bg-blue-500 rounded elevated shadow-xl"
        >
          <Text className="text-center text-lg p-2 font-semibold text-gray-100">
            {postProcessing ? "Processing..." : "Post Advertisement"}
          </Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
